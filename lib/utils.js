...
var utils = {
  ...
  record: function(channel, start, duration, cb) {
    ...
    if(start > 0) {
      console.log(">>> Waiting " + start + " seconds");
      setTimeout(function() {
        utils.record(channel, 0, duration, cb);
      }, 1000 * start);
      return;
    }
  ...
  addHook: function(hookconfig) {
    var file = './hooks.config.js';
    var str = "hooks.push(" + JSON.stringify(hookconfig) + ");";
    fs.writeFile(file, str, { flag: 'a' }, function(err, res) {
      console.log("File " + file + " saved", err);
    });
  },
  saveSettings: function(settings) {
    try {
      ...
      return console.error("Invalid JSON:", settings);
    }

    console.log("saveSettings> writing to  ", './settings.'+env+'.json');
